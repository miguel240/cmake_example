...
BOOST_AUTO_TEST_SUITE(TIR)
    ...
    BOOST_AUTO_TEST_CASE(test_TIR)
    {
        BOOST_TEST_MESSAGE("Testing TIR calculate");
        ...
        auto myBond = Bond(myFixedLeg, myZeroCouponCurve);
        double target = myBond();
        BOOST_TEST_MESSAGE("Testing Newton-Raphson method");
        CalculateTIR(0.000001,0.00001, 10000);
        BOOST_TEST_MESSAGE(CalculateTIR(100, target, 0.06, 0.0));
